...
function App() {
  ...
  useEffect(() => {
    const dynamicSlides = () => {
      const blocks = document.querySelectorAll(".ProseMirror > [data-node-type=blockGroup] > [data-node-type=block-outer]")
      const arr = Array.from(blocks);

      const i = findFirstTrueIndex(Array.from(blocks), isOffscreen);

      if (i != -1) {

        const slide = arr[i].closest('[data-node-type=blockGroup]') as HTMLElement;
        const slideRect = slide.getBoundingClientRect();
        const slideWidth = slideRect.width;
        const slideHeight = slideRect.height;

        const slideIndex = Array.from(slide.parentElement!.children).indexOf(slide);
        const slideCount = slide.parentElement!.children.length;
  ...
  // const content = chunk(getContent(), 5);
  ...
  if (isPptPreview) {
    return <div className="reveal">
      <div className="slides">
        {content.map((c, index) =>
          <section key={index}>
            <Editor editable={!isPreview && !isPptPreview} initialContent={c} />
          </section>
        )}
      </div>
    </div>
